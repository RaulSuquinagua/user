...
var CartService = /** @class */ (function () {
    function CartService(events) {
        this.events = events;
        this.cart = [];
        // total = 0;
        this.total = new rxjs__WEBPACK_IMPORTED_MODULE_2__["Subject"]();
    }
    CartService.prototype.add = function (product) {
        var _this = this;
        if (this.cart.length != 0) {
            var index = this.cart.findIndex(function (p) { return p.key === product.key; });
            if (index == -1) {
                console.log("pushing");
                product.qty = 1;
        ...
        this.calculateTotal().subscribe(function (data) {
            _this.events.publish('cart:updated', data.totalQty);
        });
    };
    ...
    CartService.prototype.increase = function (index) {
        this.cart[index].qty += 1;
        this.cart[index].total = this.cart[index].qty * parseFloat(this.cart[index].price);
        this.calculateTotal();
    };
    CartService.prototype.decrease = function (index) {
...
var CommonService = /** @class */ (function () {
    function CommonService(toast, loadCtrl) {
        this.toast = toast;
        this.loadCtrl = loadCtrl;
    }
    CommonService.prototype.showToast = function (message) {
        this.toast.create({
            message: message,
            duration: 2000
        }).then(function (d) { return d.present(); });
    };
    CommonService.prototype.showLoader = function () {
        var _this = this;
        this.loadCtrl.create({ spinner: 'dots' }).then(function (res) {
            _this.loader = res;
